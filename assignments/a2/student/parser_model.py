...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # 对 embedding hidden weight 存疑
        self.embed_to_hidden_weight = nn.Parameter(torch.empty((self.n_features * self.embed_size, self.hidden_size), dtype=torch.float32))
    ...
    def embedding_lookup(self, w):
        ...
        embeddings = self.embeddings  # (num_words, embed_size)
        
        # Flatten the word indices and gather the corresponding embeddings
        flat_indices = w.view(-1)  # (batch_size * n_features,)
        selected_embeddings = torch.index_select(embeddings, 0, flat_indices)  # (batch_size * n_features, embed_size)
        
        # 2) Reshape the tensor to (batch_size, n_features * embed_size)
        x = selected_embeddings.view(batch_size, n_features * self.embed_size)
    ...
    def forward(self, w):
        ...
        hidden = nn.functional.relu(torch.matmul( self.embedding_lookup(w), self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
