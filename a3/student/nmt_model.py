...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        ### END YOUR CODE
        # Step 1: Construct Tensor `X` of source sentences with shape (src_len, b, e)
        X = self.model_embeddings.source(source_padded)
        
        # Step 2: Apply the post_embed_cnn layer
        X = torch.permute(X, (1, 2, 0))  # Change shape to (b, e, src_len)
        X = self.post_embed_cnn(X)
        X = torch.permute(X, (2, 0, 1))  # Revert shape to (src_len, b, e)
        
        # Step 3: Compute `enc_hiddens`, `last_hidden`, `last_cell` by applying the encoder to `X`
        ...
        # Step 4: Compute `dec_init_state` = (init_decoder_hidden, init_decoder_cell)
        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim=1)  # Concatenate forwards and backwards
        init_decoder_hidden = self.h_projection(last_hidden)

        last_cell = torch.cat((last_cell[0], last_cell[1]), dim=1)  # Concatenate forwards and backwards
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            ...
            # Use the step function to compute the Decoder's next (cell, state) and new combined output o_t
        ...
        # 4. Use torch.stack to convert combined_outputs to a single tensor
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ### END YOUR CODE
        # Apply the decoder LSTM cell to Ybar_t and dec_state to obtain new dec_state
        dec_state = self.decoder(Ybar_t, dec_state)
        dec_hidden, dec_cell = dec_state
        
        # Compute the attention scores e_t
        # dec_hidden: (b, h) -> unsqueeze to (b, h, 1)
