...
def minibatch_parse(sentences, model, batch_size):
    ...
    # # new list partial_parse
    # partial_parses = []
    # for index in range(0,len(sentences)):
    #     partial_parses.append(PartialParse(sentences[index]))
    # unfinished_parses = partial_parses[:]
    # while len(unfinished_parses)!=0:
    #     tmp = unfinished_parses[:batch_size]
    #     transitions = model.predict(tmp)
    #     # apply transition
    #     # record complete index
    #     okIndex = []
    #     for index in range (0, len(tmp)):
    #         tmp[index].parse_step(transitions[index])
    #         if len(tmp[index].stack) == 1 and len(tmp[index].buffer) ==0:
    #             okIndex.append(index)
    #     # pop index
    #     tmp = remove_elements(tmp,okIndex)
    
    # for index in range (0, len(partial_parses)):
    #     dependencies.append(partial_parses[index].dependencies)

    # Step 1: Initialize partial parses for each sentence
    partial_parses = [PartialParse(sentence) for sentence in sentences]
    
    # Step 2: Create a shallow copy of partial parses to track unfinished parses
    unfinished_parses = partial_parses[:]
    
    while unfinished_parses:
        # Step 3: Get a minibatch of unfinished parses
        minibatch = unfinished_parses[:batch_size]
        
        # Step 4: Predict transitions for the current minibatch
        transitions = model.predict(minibatch)
        
        # Step 5: Apply the predicted transitions to each parse in the minibatch
        for i, p in enumerate(minibatch):
            p.parse_step(transitions[i])
            # If the parse is finished, remove it from unfinished_parses
            if len(p.stack) == 1 and len(p.buffer) == 0:
                unfinished_parses.remove(p)
    
    # Step 6: Collect the dependencies from all partial parses
